...
class RationalOptionPages {
					...
					if ( ! empty( $section_params['fields'] ) ) {
						foreach ( $section_params['fields'] as $field_key => $field_params ) {
							...
							if (
								...
								$params['title'] = "<label for='{$params["id"]}'>" . __( $params['title'], 'themeable-contact-form' ) . '</label>';
							}
	...
	protected function build_menu_page( $page_key ) {
		...
		?>
		<div class="wrap">
			<h1><?php _e( $GLOBALS['title'], 'themeable-contact-form' ); ?></h1>
														   <?php
		...
		switch ( $field['type'] ) {
			case 'checkbox':
				...
				printf(
					...
					$field['value'],                                                                    // value
					! empty( $field['text'] ) ? __( $field['text'], 'themeable-contact-form' ) : ''                                        // text
				);
				break;
			case 'media':
				...
				printf(
					...
					$field['id'],                                                                       // id
					"{$page_key}[{$field['id']}]",                                                      // name
					! empty( $field['placeholder'] ) ? 'placeholder="' . __( $field['placeholder'], 'themeable-contact-form' ) . '"' : '',     // placeholder
					$field['title_attr'],                                                               // title
					$field['value'],                                                                    // value
					! empty( $attributes ) ? implode( ' ', $attributes ) : '',                           // additional attributes
					$upload_button,                                                                     // upload button
					! empty( $field['text'] ) ? '<p class="help">' . __( $field['text'], 'themeable-contact-form' ) . '</p>' : ''              // text
				);
				break;
			case 'radio':
				echo '<fieldset><legend class="screen-reader-text">' . __( $field['title'], 'themeable-contact-form' ) . '</legend>';
				$c = 0;
				foreach ( $field['choices'] as $value => $label ) {
					...
					printf(
						...
						$field['id'],                                                                       // id
						"{$page_key}[{$field['id']}]",                                                      // name
						__( $label, 'themeable-contact-form' ),                                                                               // title
						$value,                                                                             // value
						__( $label, 'themeable-contact-form' ),                                                                               // label
			...
			case 'select':
				...
				printf(
					...
					$field['id'],                                                                       // id
					$field_tag_name,                                                        // name
					__( $field['title_attr'], 'themeable-contact-form' )                                                              // title
				);
				foreach ( $field['choices'] as $value => $text ) {
					...
					printf(
						...
						$selected,                                                                          // selected
						$value,                                                                             // value
						__( $text, 'themeable-contact-form' )                                                                             // text
					);
				}
				echo '</select>';
				break;
			...
			case 'wp_editor':
				...
				echo ! empty( $field['text'] ) ? '<p class="help">' . __( $field['text'], 'themeable-contact-form' ) . '</p>' : '';
				break;
			default:
				printf(
	...
	protected function validate_field( $field, $page_key, $section_key, $field_key, $page, $section ) {
		...
		if ( empty( $field['title'] ) ) {
			$this->submit_error( __( 'Field parameter "title" is required', 'themeable-contact-form' ) );
		}
		...
		// Title attribute.
		$field['title_attr'] = empty( $field['title_attr'] ) ? __( $field['title'], 'themeable-contact-form' ) : $field['title_attr'];

		// Choices.
		if ( empty( $field['choices'] ) && in_array( $field['type'], array( 'radio', 'select' ), true ) ) {
	...
	protected function validate_page( $page_key, $page_params, $parent_slug = false ) {
		...
		if ( empty( $page_params['page_title'] ) ) {
			$this->submit_error( __( 'Page parameter "page_title" is required', 'themeable-contact-form' ) );
		}

		// Menu title.
		if ( empty( $page_params['menu_title'] ) ) {
			$page_params['menu_title'] = __( $page_params['page_title'], 'themeable-contact-form' );
		}
	...
	protected function validate_section( $section, $page_key, $section_key, $page ) {
		...
		if ( empty( $section['title'] ) ) {
			$this->submit_error( __( 'Section parameter "title" is required', 'themeable-contact-form' ) );
		}
